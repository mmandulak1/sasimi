...
from utils.verification import verification, verification_opt, neds
from utils.utils import binary_search, binary_search_dupl, post_basic, post_positional
from edit.join_utils import transform_collection, build_stats_for_record, build_index
...
def simjoin(collection1, collection2, delta, idx, lengths_list, jointFilter, posFilter, verification_alg, log, matchAlg):
    ...
    for R in range(len(collection1['words'])):
        ...
        if R % 100 == 0:
            print("\rProgress {:,}/{:,}".format(R, len(collection1['collection'])), end='')
...
#                break
        ...
        for S, util_gathered in cands_scores.items():
            ...
            if verification_alg >= 0:
                ...
                if RLen < SLen:
                    score = verification(R_rec, S_rec, neds, pers_delta, matchAlg)
                else:
                    score = verification(S_rec, R_rec, neds, pers_delta, matchAlg)
            t2 = time()
            candver_time += t2-t1
            #print("SCORE: ",score, "DELTA CALC: ", delta - score)
...
class TokenJoin():
    ...
    def tokenjoin_self(self, df, id, join, attr=[], left_prefix='l_', right_prefix='r_', delta=0.7, jointFilter=False, posFilter=False, verification_alg=0, keepLog=False, matchAlg = 0):
        total_time = time()
        log = {}
        ...
        output = simjoin(collection, collection, delta, 
                         idx, lengths_list, jointFilter, posFilter, verification_alg, log, matchAlg)

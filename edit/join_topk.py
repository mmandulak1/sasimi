...
def delta_init(collection1, collection2, k, delta_alg, element,
               delta_generation=0.9, mu=0.01, lamda=2, sample_percentage=0.4, matchAlg = 0):
    cached = {}
    output = []
    if delta_alg == 0:
        pass
        ...
        while len(output) != k :
            ...
            for j in range(i+1, limit):
                ...
                score = verification(R_rec, S_rec, neds, 0,matchAlg)
                if i not in cached:
                    cached[i] = set()
                cached[i].add(j)
...
def simjoin(collection1, collection2, k, idx, lengths_list, delta_alg, log, matchAlg):

    selfjoin = collection1 == collection2
    init_delta = time()
    output, cached = delta_init(collection1, collection2, k, delta_alg, neds, matchAlg)
    if len(output) == 0:
        delta = 0.000000001
    else:
...
class TokenJoin():
    ...
    def tokenjoin_self(self, df, id, join, attr=[], left_prefix='l_', right_prefix='r_', k=1000, delta_alg=0, keepLog=False, matchAlg=0):
        total_time = time()
        log = {}
        ...
        output = simjoin(collection, collection, k, idx,
                         lengths_list, delta_alg, log, matchAlg)
